...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(trim($image) === '') {
        $errors['image'] = 'The image field is mandatory!';
    } else {
...
<body>
    <header>
        <h1><a href="index.php">IK-Library</a> > Add</h1>
    </header>
    ...
    <div class="wrapper d-flex align-items-stretch">
        ...
        <div id="content" class="p-4 p-md-5 pt-5">
            <div class="mb-3">
                <span class="h1 span-center">add a new book</span>
            </div>
            <form action="" method="post" enctype="multipart/form-data">
                <div class="mb-3">
                    <label for="image" class="form-label">Book cover:</label><br>
                    <input type="file" id="image" name="image"><br><span class="span-error"><?= $errors['image'] ?? '' ?></span> 
                </div>
                <div class="mb-3">
                    <label for="title" class="form-label">Title:</label>
                    <input type="text" class="form-control" id="title" name="title" value="<?= $title ?? '' ?>"><span class="span-error"><?= $errors['title'] ?? '' ?></span>
                </div>
                <div class="mb-3">
                    <label for="author" class="form-label">Author:</label>
                    <input type="text" class="form-control" id="author" name="author" value="<?= $author ?? '' ?>"><span class="span-error"><?= $errors['author'] ?? '' ?></span>
                </div>
                <div class="mb-3">
                    <label for="year" class="form-label">Year:</label>
                    <input type="text" class="form-control" id="year" name="year" value="<?= $year ?? '' ?>"><span class="span-error"><?= $errors['year'] ?? '' ?></span>                
                </div>
                <div class="mb-3">
                    <label for="planet" class="form-label">Planet:</label>
                    <input type="text" class="form-control" id="planet" name="planet" value="<?= $planet ?? '' ?>"><span class="span-error"><?= $errors['planet'] ?? '' ?></span>
                </div>   
                <div class="mb-3">
                    <label for="genre" class="form-label">Genre:</label><br>
                    <div class="custom-checkbox">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-romance" name="genre[]" value="Romance" <?= in_array('Romance', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-romance">Romance</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-adventure" name="genre[]" value="Adventure" <?= in_array('Adventure', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-adventure">Adventure</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-thriller" name="genre[]" value="Thriller" <?= in_array('Thriller', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-thriller">Thriller</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-fantasy" name="genre[]" value="Fantasy" <?= in_array('Fantasy', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-fantasy">Fantasy</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-ya" name="genre[]" value="Young Adult" <?= in_array('Young Adult', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-ya">Young Adult</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-mystery" name="genre[]" value="Mystery" <?= in_array('Mystery', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-mystery">Mystery</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-historical" name="genre[]" value="Historical" <?= in_array('Historical', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-historical">Historical</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-horror" name="genre[]" value="Horror" <?= in_array('Horror', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-horror">Horror</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-scifi" name="genre[]" value="Science Fiction" <?= in_array('Science Fiction', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-scifi">Science Fiction</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-humorous" name="genre[]" value="Humorous" <?= in_array('Humorous', $genre ?? []) ? 'checked' : '' ?>>
                            <label class="custom-control-label" for="genre-humorous">Humorous</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="genre-western" name="genre[]" value="Western" <?= in_array('Western', $genre ?? []) ? 'checked' : '' ?>>
                ...
                <div class="mb-3">
                    ...
                    <textarea id="description" name="description" rows="5" style="display: block; width: 100%;"><?= $description ?? '' ?></textarea><span class="span-error"><?= $errors['description'] ?? '' ?></span>
                </div>
                <div class="mb-3">
